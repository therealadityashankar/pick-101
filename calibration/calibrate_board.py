"""calibrate_board.py — Live homography viewer + perspective calibration.
...
Calibration mode (press C):
    ...
    After all 4 corners are recorded it fits a per-axis linear correction
    and saves it to .calibration/camera_calibration.npz, which
    run_real_ik.py and visualize_irl_block.py load automatically.

Usage:
    uv run python calibration/calibrate_board.py
    uv run python calibration/calibrate_board.py --camera 1 --block-tag 101 \\
...
SAVE_PATH = Path(".calibration/camera_calibration.npz")
...
def fit_linear_2d(calib_data):
    ...
    np.savez(SAVE_PATH, Sx=Sx, Bx=Bx, Sy=Sy, By=By,
             measurements=arr)
    print(f"\nSaved → {SAVE_PATH}")
    print("run_real_ik.py and visualize_irl_block.py load this file "
...
def main():
    ...
    while True:
        ...
        if calib['done']:
            status = "Calibration complete — saved to .calibration/.  C=redo  Q=quit"
            color  = (0, 220, 0)
